...
class DNN_SKalmanNet_GSS(torch.nn.Module):
    def __init__(self, x_dim: int = 2, y_dim: int = 2):
        ...
        # GRU output -> H2 -> Pk
        self.l2 = nn.Sequential(nn.Linear(self.gru_hidden_dim, H2), nn.ReLU(), nn.Linear(H2, self.output_dim_1))
        ...
        # GRU output -> H2 -> Sk
        self.l4 = nn.Sequential(nn.Linear(self.gru_hidden_dim, H2), nn.ReLU(), nn.Linear(H2, self.output_dim_2))
...
class DNN_KalmanNet_GSS(torch.nn.Module):
    def __init__(self, x_dim: int = 2, y_dim: int = 2):
        ...
        # input layer
        self.l1 = nn.Sequential(nn.Linear(self.input_dim, H1), nn.ReLU())
        ...
        self.l2 = nn.Sequential(nn.Linear(self.gru_hidden_dim, H2), nn.ReLU(), nn.Linear(H2, self.output_dim))
...
class KNet_architecture_v2(torch.nn.Module):
    def __init__(self, x_dim: int = 2, y_dim: int = 2, in_mult=20, out_mult=40):
        ...
        self.FC2 = nn.Sequential(
            nn.Linear(self.d_input_FC2, self.d_hidden_FC2), nn.ReLU(), nn.Linear(self.d_hidden_FC2, self.d_output_FC2)
        )
