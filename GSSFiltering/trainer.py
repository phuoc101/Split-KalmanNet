...
class Trainer:
    ...
    def __init__(
        self, dnn: Union[KalmanNet_Filter, Split_KalmanNet_Filter, KalmanNet_Filter_v2], data_path, save_path, mode=0
    ):
        # Example:
    ...
    def train_batch_alternative(self):
        ...
        if self.train_count % save_num == 0:
            try:
                torch.save(
                    self.dnn.kf_net, "./.model_saved/" + self.save_path[:-3] + "_" + str(self.train_count) + ".pt"
                )
            except:
                print("here")
                pass
